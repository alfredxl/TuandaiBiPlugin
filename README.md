...
```
在TuandaiBiPlugin类中的actionPerformed方法中，可以对整个项目，甚至IEAD做出一些更改， 这里仅仅  
>演示如何生成java代码：  
```java
public void actionPerformed(final AnActionEvent e) {
        // 获取Project
...
```     
获取Note类中是否已经有Sensors之类，如果没有，则创建，有则直接或者，创建的时候可以直接根据字符串创建（这个相当方便），     
有几点需要注意：    
...
2、这里采取了巧妙的手段，直接在这个匿名类中，生成了一个内部类，我们只把这个内部类取出来即可；   
...
## 总结：   
关于IDEA插件的编写，这里也是初步入门的示例，总结了一些代码自动生成方面的注意事项，因为官方文档基本没有，    
